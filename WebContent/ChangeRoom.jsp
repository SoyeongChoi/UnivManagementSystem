...
<h2><%=className %> 일정 수정</h2>
	...
	<input type="hidden" name = "eTime" value="<%=eTime%>">
			...
			<tr>
				<td>강의실 : </td>
				<td><%=roomId%></td>
				<td><select id="select" name="changeRoomId">
						<option value="changeRoomId" disabled>강의실 선택</option>
						<% for(int i = 0; i < arrRoom.length; i++){
								if(arrRoom[i].equals(roomId)){
							%>
						<option value="<%=arrRoom[i]%>" selected="selected"><%=arrRoom[i]%></option>
								<%}else{
								%>
						<option value="<%=arrRoom[i]%>"><%=arrRoom[i]%></option>
